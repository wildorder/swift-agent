...
function requireEnv(): { baseUrl: string; apiKey: string } {
  ...
  if (!apiKey) missing.push('SMOKE_API_KEY');
  if (!baseUrl || !apiKey) {
...
async function main(): Promise<void> {
  ...
  console.log(
    `realtime smoke: base=${baseUrl} agent=${AGENT_NAME} requireTokens=${REQUIRE_TOKENS} budget=${OVERALL_BUDGET_MS}ms`,
  );
  ...
  if (passed) {
    console.log('realtime smoke: PASSED — message_started → token(s) → message_completed');
    process.exit(0);
  }
